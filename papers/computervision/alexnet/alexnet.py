...
from jonigrad.layers import Conv, ReLU, Linear, LRNorm, MaxPool, Dropout, Flatten, Module

class AlexNet(Module):
    def __init__(self):
        self.conv1 = Conv(in_channels=1, out_channels=32, kernel_size=5, stride=1, padding=1)       
        self.conv2 = Conv(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.conv3 = Conv(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.conv4 = Conv(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.conv5 = Conv(in_channels=64, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.relu1 = ReLU()
        self.relu2 = ReLU()
        self.relu3 = ReLU()
        self.relu4 = ReLU()
